...
export function PostInfo({ title, date, tags }: PostInfoProps) {
  return (
    <div className="mb-5">
      <h2 className="not-prose font-heading my-0 w-max text-4xl">{title}</h2>
      <div className="w500:flex-col w500:items-start flex items-center justify-between">
        <div className="flex items-center">
          ...
          <p className="text-sm leading-5 tracking-wide">{date}</p>
        </div>
        <div className="flex items-center">
          {tags.map((tag: string) => (
            <ButtonLink
              key={tag}
              className="not-prose rounded-base border-border bg-main text-main-foreground ml-3 border px-2 py-0.5 text-sm no-underline first:ml-0"
              href={`/tags/${tag}`}
            >
              #{tag}
            </ButtonLink>
